...
var col_color: Color = Color (1,1,1,1)
...
func _on_click_pressed() -> void:
	...
	_check_store_color_yellow()
	if resource >= price_brain:
		btn_brain.set_modulate(col_yellow)
	
func _check_store_color_yellow() -> void:
	if (resource >= price_brain): _color_store_brain(col_yellow)
	if (resource >= price_eye): _color_store_eye(col_yellow)
	if (resource >= price_lungs): _color_store_lungs(col_yellow)
	if (resource >= price_rib_cage): _color_store_rib_cage(col_yellow)
...
func _on_button_eye_pressed() -> void:
	...
	_btn_part_pressed(btn_eye)
func _on_button_lungs_pressed() -> void:
	img_lungs.visible = true
	_btn_part_pressed(btn_lungs)
func _on_button_rib_cage_pressed() -> void:
	img_rib_cage.visible = true
	_btn_part_pressed(btn_rib_cage)

func _btn_part_pressed(button: Button) -> void:
	for b in btn_array:
		if b == button:
			b.set_modulate(col_blue)
		else:
			b.set_modulate(col_green)
...
func _on_btn_upg_brain_pressed() -> void:
	if resource >= price_brain:
		if (lvl_brain == 0):
			btn_brain.set_modulate(col_blue)
